...
//status = cublasAlloc(M*N, sizeof(cuComplex), (void**)&d_A);
//status = cublasAlloc(M*N, sizeof(cuComplex), (void**)&d_B);
//status = cublasAlloc(M*N, sizeof(cuComplex), (void**)&d_C);
...
h_B[iboucle].y=mat_real[iboucle];
  ...
  dureetot=dureetot+dureeloc;
		...
		printf("Timeur CUDA sans transfert = %f ms\n",time1);


  printf("Temps clock avec transfert = %f ms\n",dureeloc*1000);
